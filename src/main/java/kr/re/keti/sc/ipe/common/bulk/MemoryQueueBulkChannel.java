package kr.re.keti.sc.ipe.common.bulk;
...
import kr.re.keti.sc.ipe.common.exception.CoreException;
...
public class MemoryQueueBulkChannel<T> implements IBulkChannel<T> {
	...
	/** 로거클래스 */
	private final Logger logger = LoggerFactory.getLogger(MemoryQueueBulkChannel.class);
	...
	@Override
	public void start() {
		logger.info("Start BulkProcess. name=[" + instanceName + "], threadCount=[" + workThreadCount + "], bulkIntervalMillis=[" + bulkIntervalMillis + "], queueSize=[" + queueSize + "]");
		for(int i=0; i<workThreadCount; i++) {
			try {
				if(executeMode == ExecuteMode.ASYNC_BULK) {
					ConsumeBulkThread consumeThread = new ConsumeBulkThread();
					logger.info("Start BulkProcess. name=[" + instanceName + "-" + i + "]");
				...
				logger.error("MemoryQueueBulkChannel start ERROR. name=" + instanceName, e);
			}
		}
	}
	...
	public void stop() {
		for(int i=0; i<workThreadCount; i++) {
			try {
				logger.info("Stop BulkProcess. name=[" + instanceName + "-" + i + "]");
				workThreads[i].interrupt();
				workThreads[i] = null;
			} catch (Exception e) {
				logger.error("MemoryQueueBulkChannel stop ERROR. name=" + instanceName, e);
			}
		}
	}
	...
	public void clear(boolean storeRemainingData) {
		clear(storeRemainingData, logger);
	}

	@Override
	public void clear(boolean storeRemainingData, Logger logger) {
		this.logger.info("Clear MemoryQueueBulkChannel queue. Name=[" + instanceName + "], StoreRemainingData=[" + storeRemainingData + "]");
		if(storeRemainingData && queue.size() > 0) {
			...
			if(objList.size() > 0) {
				this.logger.info("Clear Data size=[" + objList.size() + "]");
				for (int i=0; i<objList.size(); i++) {
					logger.info("Clear Data=[" + instanceName + "-" + i + "]. Object=" + objList.get(i).toString());
				}
			} else {
				this.logger.info("Clear Data size=[0]");
			}
		} else {
			this.logger.info("Clear Data size=[" + queue.size() + "]");
			queue.clear();
		}
	}
		...
		public void run() {
			while(!Thread.currentThread().isInterrupted()) {
				try {
					...
					logger.error("Process error. name=[" + instanceName + "]", e);
				}
